...
test(`Test at kjente tags rendrer i språkkomponent`, () => {
    const tekster = {
        testtekst: 'Dette er en test. {linjeskift} <b>Fet</b><punktliste><punkt>Listeelement</punkt></punktliste>',
    };
    ...
    ['b', 'br', 'ul', 'li'].forEach(støttaTag => expect(document.querySelector(støttaTag)).toBeInTheDocument());
});
...
test(`Test at ekstra tags kan legges til språkkomponent via props`, () => {
    ...
    ['b', 'br', 'em'].forEach(støttaTag => expect(document.querySelector(støttaTag)).toBeInTheDocument());
});
